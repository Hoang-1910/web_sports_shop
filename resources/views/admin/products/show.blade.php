    ...
    <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg p-8">
        ...
        <div class="flex gap-8">
            <div id="main-image-block">
                <img id="main-product-image" src="{{ $product->image ? asset('storage/' . $product->image) : '' }}"
                    alt="{{ $product->name }}"
            ...
            <div class="flex-1 space-y-4">
                ...
                <div>
                    ...
                    <span>{{ $product->brand->name ?? 'Không có' }}</span>
                </div>
                <div>
                ...
                <div>
                    ...
                    <span
                        class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-semibold">{{ $product->discount }}%</span>
                </div>
                <div>
        ...
        <div class="mt-8 flex justify-end">
            <a href="{{ route('admin.products.index') }}"
                class="px-5 py-2 rounded bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold transition">Quay lại</a>
            <a href="{{ route('admin.products.edit', $product->id) }}"
                class="ml-4 px-5 py-2 rounded bg-yellow-100 hover:bg-yellow-200 text-yellow-700 font-semibold transition">Sửa</a>
        </div>
    </div>
@endsection
    ...
    <script>
        ...
        document.querySelectorAll('.size-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                document.querySelectorAll('.size-btn').forEach(b => b.classList.remove('bg-purple-200',
                    'text-purple-900'));
        ...
        document.querySelectorAll('.color-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                document.querySelectorAll('.color-btn').forEach(b => b.classList.remove('bg-purple-200',
                    'text-purple-900'));
                this.classList.add('bg-purple-200', 'text-purple-900');
