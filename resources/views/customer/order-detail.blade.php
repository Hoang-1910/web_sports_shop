                ...
                <div class="card shadow-sm rounded-4 p-4">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                        ...
                        <div class="text-md-end">
                            <div class="mb-1"><span class="text-muted">Tổng tiền:</span> <span class="fw-bold text-danger fs-5">{{ number_format($order->total_amount ?? $order->total) }}đ</span></div>
                            <div class="mb-1"><span class="text-muted">Phương thức:</span> {{ $order->payment_method }}</div>
                        </div>
                    </div>
                    <hr>
                    <div class="mb-4">
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex align-items-center gap-3">
                                                <img src="{{ asset('storage/' . $item->variant->product->image) }}" alt="{{ $item->variant->product->name }}" class="rounded-3 border" width="60" height="60">
                                                <span class="fw-semibold">{{ $item->variant->product->name }}</span>
                                            </div>
                                        </td>
                    ...
                    <div class="mt-4 text-center">
                        <a href="{{ route('customer.orders') }}" class="btn btn-outline-danger px-4"><i class="bi bi-arrow-left me-2"></i>Quay lại danh sách đơn</a>
                    </div>
                </div>
            </div>
        </div>
